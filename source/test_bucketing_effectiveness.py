...
for ds in grouped_datasets:
  print(f"Bucket size: {len(ds)}")
  
# create a prompt to select training index

training_ds = grouped_datasets[2 + 1]

training_ds = training_ds.map(get_tokens, batched=True, batch_size=8)
training_ds = training_ds.rename_column("label", "labels")

training_ds_split = training_ds.train_test_split(test_size=0.15, seed=42)

training_train_ds = training_ds_split["train"]
training_eval_ds = training_ds_split["test"]

training_train_ds_split = training_train_ds.train_test_split(test_size=0.1, seed=42)

train(training_train_ds_split["train"], training_train_ds_split["test"], training_eval_ds, "Bucket 2 to 2")
